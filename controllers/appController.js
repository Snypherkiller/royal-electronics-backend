import UserModel from '../models/Puser.module.js'
...
export async function register(req,res){
  ...
  //console.log('req.body:', req.body);
    ...
    try {
        const { username, password, profile, email , firstname , lastname , address , mobile} = req.body;        
        ...
        if (existUsername || existEmail) {
          ...
          if (password) {
            ...
            res.status(201).send({ msg: "User Register Successfully", data: result });
          }
...
export async function updateUser(req,res){
    try {
        ...
        if(userId){
            ...
            // update the data
            UserModel.findOneAndUpdate({username:userId}, body).then((err, data)=>{
                if(err) throw err;
